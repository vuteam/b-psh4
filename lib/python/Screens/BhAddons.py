...
class Nab_downArea(Screen):
    ...
    def KeyOk(self):
        ...
        if self.sel == 1:
            ...
            if getBoxType() in ('sparkreloaded'):
        ...
        if self.url == 'feeds':
            if getBoxType() in ('vusolo2'): 
                ...
                nobox = self.session.open(MessageBox, _('Sorry, Connection Failed. This Plugins Are Not CompaTible With Your Box. Please press The Blue Button From BluePanel BlackHole And Press Red Button To Download From Feeds'), MessageBox.TYPE_INFO)
        else:
